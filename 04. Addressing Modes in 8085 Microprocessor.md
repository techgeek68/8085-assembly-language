...
---
## Summary Table of All 5 Addressing Modes

| # | Addressing Mode | Where the Data Is | Instruction Size | Example |
|---|---|---|---|---|
| 1 | Immediate | Inside the instruction itself (constant) | 2 or 3 bytes | `MVI A, 45H` / `LXI H, 2050H` |
| 2 | Register | Inside a CPU register | 1 byte | `MOV A, B` / `ADD C` |
| 3 | Direct | In memory; 16-bit address given in instruction | 3 bytes | `LDA 2050H` / `STA 3000H` |
| 4 | Register Indirect | In memory; address held in a register pair (HL, BC, DE) | 1 byte | `MOV A, M` / `LDAX B` |
| 5 | Implied / Implicit | Implied by the instruction; usually Accumulator or flag | 1 byte | `CMA` / `RLC` / `HLT` |
...
---
##  Complete Program Using All Five Addressing Modes
