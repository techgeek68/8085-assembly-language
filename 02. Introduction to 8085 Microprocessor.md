...
**Machine Code:** The binary (or hexadecimal) codes that the CPU directly understands and executes. Every instruction is stored in memory as one, two, or three bytes of machine code.

**Assembly Language:** A human readable symbolic representation of machine code. Mnemonics (short abbreviations) replace binary opcodes, and labels replace numeric addresses.
